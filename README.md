...
```
Служит для описания товаров и данных пользователя для отправки на сервер, для выполнения заказа
...
`container: HTMLElement` - поле для храения корневого DOM элемента компонента.  
`counterElement: HTMLElement` - поле для хранения ссылки на DOM элемент, представлющий из себя счётчик товаров в корзине.  
`basketButton: HTMLButtonElement` - поле для хранения ссылки на кнопку, открывающую корзину.  

Методы класса:  
`set counter(value: number)` - метод класса, устанавливающй значение на иконке корзины (количество товаров в корзине).  

#### Класс ViewGallery  
Является классом представления, реализующим каталог/галерею на главной странице.  
...
`events: IEvents` - поле для хранения ссылки на диспетчер событий  
`container: HTMLElement` - поле для храения корневого DOM элемента компонента.  
`catalogElement: HTMLElement` - поле для хранеия ссылки на каталог товаров.  

Методы класса:  
`set catalog(items: HTMLElement[])` - метод классы, устанавливающий список карточек для отображения на главной странице.  

#### Класс ViewModal  
Является классом представления, реализующим отображения модального окна.  
...
`events: IEvents` - поле для хранения ссылки на диспетчер событий  
`container: HTMLElement` - поле для храения корневого DOM элемента компонента.  
`contentElement: HTMLDivElement` - поле класса, хранящее ссылку на содержимое модального окна.  
`closeButton: HTMLButtonElement` - поле класса, хранящее ссылку на кнопку закрытия модального окна.  

Методы класса:  
`set content(item: HTMLElement | '')` - метод установки содержмого в модальное окно.  

#### Класс Card  
Является базовым классом, представлющим из себя элемент карточки товара.  

Конструктор:  
`constructor(container: HTMLElement)` - принмает ссылку на корневой DOM-элемент, отвечающий за отображения карточки.  

Поля класса:  
`container: HTMLElement` - поле, хранящее в себе ссылку на разметку карточки.  
`titleElement: HTMLHeadElement` - поле, хранящее ссылку на DOM-элемент, отвечающий за название товара.  
`priceElement: HTMLSpanElement` - поле, хранящее ссылку на DOM-элемент, отвечающий за цену товара.  

Методы класса:  
`set title(value: string)` - метод для установки названия товара.  
`set price(value: string` - метод для установки цены товара.  

#### Класс CardBasket  
Является классом представленя для карточки товара в корзине. Наследуется от класса `Card`.    

Конструктор:  
`constructor(events: IEvents, container: HTMLElement,     actions?: ICardActions)` - принимет ссылку на диспетчер событий, сыылку на корневой DOM-элемент, отвечающий за отображения карточки, и ссылку на объект с действиями, передаваемыми извне (из презентера).  
...
`deleteButtonElement: HTMLButtonElement` - поле, хранящее ссылку на кнопку удаления товара из корзины.  
`indexElement: HTMLSpanElement` - поле, хранящее ссылку на DOM-элемент, отвечающий за порядковый номер товара в корзине.  

Методы класса:  
`set index(value: number)` - метод установки индекса позиции элемента в корзине.  
